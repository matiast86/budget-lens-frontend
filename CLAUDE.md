...
│   │       │                          # breakdown edit opens BreakdownEditor in a modal; props: ledgerId, currency, paymentMethods, categories, groups
│   │       │                          # header buttons: "Project ahead" → ProjectExpensesModal (projectMutation), "Track balance" → CreateBalanceModal (createBalanceMutation); both invalidate ["transactions", ledgerId]
...
│   │       ├── ProjectExpensesModal.tsx # plain-useState modal: category + tag + "plan through" month; collapsible increase % / every-N-months / starting-bump %; optional note
│   │       │                          # silent defaults: currency = ledger currency, paymentMethodId = paymentMethods[0].id (backend fallback, rarely used)
│   │       │                          # UI % → API fraction (÷100, like the FIXED-bundle create flow); no month-count preview — start month is server-derived
...
│   │   │                              # projectFixedExpenses(ledgerId, ProjectFixedExpensesData, token) → POST /transactions/ledgers/:id/projections, returns ProjectedSeries[] (nests bundleTo/increaseRate/seedIncreaseRate/increaseEveryMonths under fixedBundleDto)
...
19. ~~**Fixed-expense projection ("project ahead")**~~ — backend `projectFixedExpensesForward` (`POST /transactions/ledgers/:id/projections`) wired via a "Project ahead" button next to "Track balance" in `WeeklyView`'s header → `ProjectExpensesModal` (plain-useState, sibling of `CreateBalanceModal`): category + tag selects, a "plan through" month, a collapsible block for increase % / every-N-months / starting-bump %, optional note. Silent defaults: `currency` = ledger currency, `paymentMethodId` = `paymentMethods[0].id` (a backend fallback that almost never fires — each source row keeps its own method). UI collects whole percent → service divides by 100 into a fraction and nests `bundleTo` + the rate fields under `fixedBundleDto` (matching the `ProjectionsDto` shape). No month-count preview — the start month is the server-derived frontier (month after the last already-planned one). `useMutation` in `WeeklyView` invalidates `["transactions", ledgerId]`. `WeeklyView` gained `categories` / `groups` props; both call sites pass `ledger.categories` / `ledger.groups`. `ProjectFixedExpensesData` / `ProjectedSeries` types + `projectFixedExpenses()` in `transaction-service.ts`. i18n `projection.modal.*` + `transaction.weekly.projectExpenses` in `ledger.json` (EN+ES). Group-wide rate is the backend's deliberate v0 — per-line rate/cadence config + a preview screen is a later step.
